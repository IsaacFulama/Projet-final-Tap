...
```json
{
  ...
  "whatsapp_reports": {
    ...
    "report_types": ["en_regle", "litigieux"]
  }
}
```
...
- Les rapports mensuels WhatsApp sont désormais alignés sur les enregistrements de la table `paiements`.
- Cela garantit que les données PDF reflètent correctement l'état du mois courant et évite les envois vides lorsque des paiements existent.
...
### Validation de la configuration WhatsApp
Pour que l’envoi réel fonctionne, définissez les variables d’environnement avant d’exécuter l’application :

```powershell
$env:TAP_WHATSAPP_ENABLED = "1"
$env:TAP_WHATSAPP_MODE = "cloud"
$env:TAP_WHATSAPP_TO = "+243852382067"
$env:TAP_WHATSAPP_TOKEN = "EAAB..."
$env:TAP_WHATSAPP_PHONE_NUMBER_ID = "123456789012345"
```

Ensuite, vérifiez que `config.json` contient bien :
- `whatsapp_reports.enabled: true`
- `whatsapp_reports.send_monthly_pdf: true`
- `whatsapp_reports.recipients` non vide

Si le mode est `cloud`, le token et le `phone_number_id` doivent être présents.

### Résultat attendu
- en mode `--preview` : le rapport est généré sans envoi réel
- en mode réel : le message doit être transmis à WhatsApp et le rapport doit être joint
- si le mode est `disabled` ou si les variables ne sont pas définies, le script ne fera pas d’envoi réel
...
- Issues du projet: https://github.com/IsaacFulama/Projet-final-Tap/issues
