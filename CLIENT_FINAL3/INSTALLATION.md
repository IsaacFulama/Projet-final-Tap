...
## Nouveautés Version 3.8

- Lorsqu'un souscripteur `Spécial` a un ou plusieurs mois impayés, tout nouveau versement est affecté automatiquement au plus ancien mois dû.
- Si le versement dépasse ce mois, le surplus est affecté au mois suivant, et ainsi de suite.
- Cette règle est appliquée lors d'un nouvel enregistrement et avec l'action `Ajouter paiement`.

## Nouveautés Version 3.9

- **Basculement Spécial manuel** : La reconduction mensuelle des souscripteurs `Spécial` ne se fait plus automatiquement à l'ouverture de l'application. Elle est désormais contrôlée manuellement via un choix de mois cible dans l'interface "Souscripteurs spéciaux".
- La reconduction automatique est conservée pour les souscripteurs `Simple`.
...
### Basculement mensuel (Souscripteurs Spécial)
1. Aller dans l'onglet "Enregistrements" ou utiliser le bouton "Souscripteurs spéciaux".
2. Dans la barre "Basculement mensuel Spécial", sélectionner le mois cible (à partir de 10/2025).
3. Cliquer sur "Basculement mensuel".
4. L'application crée les nouvelles lignes `En attente` pour le mois choisi et bascule les anciens paiements `En attente` en `Litigieux`.
5. Le journal interne empêche les doublons si l'opération est lancée plusieurs fois pour un même mois.
