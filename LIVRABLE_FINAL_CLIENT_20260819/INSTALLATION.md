...
Les enregistrements ne sont pas inclus dans l'EXE : ils sont stockés dans
MySQL. Le script d'initialisation sert à créer le schéma ; pour retrouver les
données du poste source, il faut restaurer une sauvegarde SQL.
```sql
...
2. Placer `TAP_Gestion_Loyers.exe` et `config.json` dans le même dossier
3. Démarrer MySQL/MariaDB avant l'application
...
Pour partager les mêmes enregistrements entre plusieurs ordinateurs, utilisez
un serveur MySQL commun et remplacez `localhost` par son adresse réseau dans
`config.json`. Avec `localhost`, chaque ordinateur utilise sa propre base.
...
Ne copiez donc pas uniquement l'EXE : cela ne transporte pas les données.
